...
from networks.EMGHandNet import EMGHandNetSupCon
...
def parse_option():
    ...
    opt.pkl_path = './save/SupCon/{}_pkl'.format(opt.dataset)
    ...
    opt.model_name = '{}_lr_{}_bsz_{}_temp_{}_tri_{}'.\
        format(opt.model, opt.learning_rate, opt.batch_size, opt.temp, opt.trial)
...
def set_loader(opt):
    ...
    if opt.dataset == 'nina1':
        if opt.subjects:
            train_dataset = Nina1Dataset(train, mode='subjects', transform=TwoCropTransform(train_transform))
        else:
            train_dataset = Nina1Dataset(train, mode='labels', transform=TwoCropTransform(train_transform))
    elif opt.dataset in ['nina2', 'nina4']:
        if opt.subjects:
            train_dataset = Nina2Dataset(train, sampled = opt.sampled, mode='subjects', transform=TwoCropTransform(train_transform))
        else:
            train_dataset = Nina2Dataset(train, sampled = opt.sampled, mode='labels', transform=TwoCropTransform(train_transform))
    else:
...
def set_model(opt):
    ...
    if opt.sampled:
        model = EMGHandNetSupCon(data = f'{opt.dataset}_sampled')
    else:
        model = EMGHandNetSupCon(data = opt.dataset)
...
def main():
    ...
    print('start with lr_{} bsz_{} temp_{}'.format(opt.learning_rate, opt.batch_size, opt.temp))
